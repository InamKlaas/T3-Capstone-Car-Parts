...
class AdminControllerTest {
    ...
    void a_create() {
        ...
        ResponseEntity<Admin> response =
                restTemplate.postForEntity(url, admin, Admin.class);
    ...
    void d_update() {
        ...
        String readUrl = BASE_URL + "/" + admin.getUserid();
        ResponseEntity<Admin> response =
                restTemplate.getForEntity(readUrl, Admin.class);

        assertNotNull(response.getBody());
        admin = response.getBody();
        System.out.println("Updated Admin: " + admin);

        assertEquals("Moderator", admin.getRole());
    }
